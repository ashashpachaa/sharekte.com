...
import { Users, Settings, LogOut, BarChart3, AlertCircle, Building2 } from "lucide-react";
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-50 border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="container max-w-7xl mx-auto flex h-16 items-center justify-between px-4">
          <div>
            <h1 className="text-2xl font-bold text-primary">BusinessCorp Admin</h1>
            <p className="text-xs text-muted-foreground">
              {currentAdmin.name} ({currentAdmin.role.replace("-", " ").toUpperCase()})
            </p>
          </div>
          ...
          <div className="bg-card border border-border/40 rounded-lg p-6">
            <div className="flex items-center justify-between">
              ...
              <div className="w-12 h-12 rounded-full bg-green-100 flex items-center justify-center">
                <span className="text-green-600 font-bold">{activeUsers > 0 ? "✓" : "-"}</span>
              </div>
            </div>
          </div>
        ...
        <div className="mt-8 bg-card border border-border/40 rounded-lg p-6">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between items-center py-2 border-b border-border/20">
              ...
              <span className="font-semibold text-orange-600">{suspendedUsers}</span>
            </div>
